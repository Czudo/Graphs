...
class ERGraph(Graph):
    def __init__(self, N, p):
        super().__init__() #create random graph as Graph structure
        self.N = N
        self.p = p
        ...
        allEdges = [i for i in itertools.combinations(vertices, 2)]  #list of all posible conections
        self.edges = [allEdges[x] for x in range(1, max) if U[x] <= p]  #get conections with random number larger than p
        self.addEdgesFromList(self.edges)
